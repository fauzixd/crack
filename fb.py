...
#------------------[ LOGO-LAKNAT ]-----------------#
def banner():
	print(f'\t   {m}WELCOME TO MY TOOLS ')
#  print(f'\t THE MEMEKERS  ')
	print(f'\t  Author <•> cracker')
	print(f'\t   {b}From <•> INDONESIA ')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
# gh = 'github.com/Al-Vino'
	cetak(nel('\tSelamat Datang [green]%s[cyan] :)'%(my_name)))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Sedang mebak pasword acak {m}•{k}•{h}•{x} <<<<< ')
	print('')
		...
		try:
			...
			#	os.popen('play-audio data/cp.mp3')
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/4MBF-DATA/OK/'+okc,'a').write(idf+'|'+pw+'\n')
